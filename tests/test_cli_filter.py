...
def _invoke(tmp_path: Path, monkeypatch, args: list[str], *,
    ...
    # imports `from auto_applier.pipeline import FilterWorker`; pipeline re-exports the same
    # symbol it imports from filter_worker, so monkeypatching both keeps the test
    # robust to the CLI changing its import style.
    monkeypatch.setattr("auto_applier.pipeline.FilterWorker", _RecordingWorker, raising=True)
    monkeypatch.setattr(
        "auto_applier.pipeline.filter_worker.FilterWorker", _RecordingWorker, raising=True
    )
