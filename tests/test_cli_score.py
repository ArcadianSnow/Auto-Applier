...
def _invoke(tmp_path: Path, monkeypatch, args: list[str], *,
    ...
    monkeypatch.setattr("auto_applier.pipeline.ScoreWorker", _RecordingWorker, raising=True)
    monkeypatch.setattr(
        "auto_applier.pipeline.score_worker.ScoreWorker", _RecordingWorker, raising=True
    )
