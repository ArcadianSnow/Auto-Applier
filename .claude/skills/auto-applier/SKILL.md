...
| **Field coverage ROUND 2 (✅ SHIPPED 2026-06-24)** — fact-bank `languages`(default English) + `availability`; classifiers LANGUAGES (programming-guarded)/CURRENT_COMPANY(→work_history[0])/AVAILABILITY, all classify BEFORE `is_open_ended`; owner defaults notice+availability→"2 weeks", languages→English applied in the RESOLVER (explicit onboarding value wins); onboarding `merge_extras`+`/onboarding/extras` capture languages/availability + route salary→`user_config.targeting`; wizard "More details" step. Ashby id-less combobox FILL via `fill_resolutions` `combobox_fill` hook + `ashby_apply.fill_ashby_combobox` (re-derives field-entry by synthetic `ashby_q<n>` index; types city token; clicks best `[role=option]`, city-token-required else bail). **Live-verified end-to-end** on a Ramp Ashby form (Playwright MCP, read-only). 1430 green. Deferred: Lever/Ashby smoke tests. | `research/field-coverage-round2.md` |
