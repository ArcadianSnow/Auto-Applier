...
| **E2 "fill what it can on demand" — ✅ PHASES A+B SHIPPED 2026-06-24** (Phase C = owner-watched live, not autonomously runnable): `POST /api/jobs/{id}/assisted/prepare` reuses the scheduler's ApplyWorker via **`prepare_single(job_id, *, page)`** (built on ONE shared `@stage("apply")` `_drive_and_record` path refactored out of `_process_one` — no drift; forces assisted+real; single-flight 409 via `_prepare_lock`/`prepare_in_progress`; any failure ends in REVIEW), launcher **`open_page`** returns the live page + engages the takeover, `cli serve` `_worker_holder`→`app.state.apply_worker_holder`, **"Fill what it can"** dashboard button heads the "Needs your decision" group (`fillOnDemand` → "filled N / M left"). Promotes a REVIEW job into the "Ready to finish" (ASSISTED_PENDING) lane. 3 owner Qs resolved with the doc's recommendations (REVIEW+QUEUED_APPLY only; résumé-less fills the rest; one-line filled/left). `PrepareSingleError(code)` maps 404/422/409. 1521 green. | `research/e2-on-demand-fill-design.md` |
...
| **Fill mechanics ROUND 3 — fail-closed scoping + option snapping (✅ SHIPPED 2026-07-31)** — Rounds 1–2 fixed *discovery* and *classification*; Round 3 fixes **whether the right answer lands on the right control**. Five defects CONFIRMED in a real headless Chromium (fixtures, read-only, never submitted): **F1** `_OPTION_GROUP_CLICK_JS` fell back to `scope = document` when `field_id` resolved to nothing — and Ashby's synthetic `ashby_q<n>` ids NEVER resolve, so the sponsorship "No" clicked the **work-auth** question's "No" (applicant recorded as not authorized; both reported as filled). Container now resolves by name/id → Ashby index (title-confirmed) → question label → bounded 4-ancestor walk; **no container ⇒ bail**. **F2** a checkbox discovered as `kind='input'` was TYPED into, and `human_type` clicks to focus → a "No" answer **checked the box**; `_control_kind` now probes the live element and routes choice controls to the option clicker (Greenhouse is the reachable case — it doesn't type its choice widgets). **F3** `settle_open_dropdown`'s loose substring match committed "Nope, never used it" for "No" → now delegates to `_click_combobox_option`. **F4** new `snap_to_option` maps canonical answers onto the form's own prose ("Yes" → "Yes, I am authorized to work in the US"; `select_option` needs an EXACT value/label match) with the same conservative ladder; ambiguous ⇒ bail. **F5** `_locate_ashby_combobox` confirms its positional entry against the question title (a revealed conditional field shifts every later index). **New invariant: a fill must be provably scoped to its question — `filled == True` is not evidence of a correct fill unless container-scoped.** Still open: GH radio/checkbox grouping, options never shown to the LLM tier, **no post-fill read-back verification (highest-value follow-up)**, Lever/Ashby smoke. | `research/fill-mechanics-hardening.md` |
