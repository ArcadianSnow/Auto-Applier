...
from av3.cli.main import _wait_for_port, cli
...
class TestLaunchCmd:
    ...
    def _run(self, args, *, settings, child_returncode=0,
        ...
        with patch("subprocess.Popen", _fake_popen), \
             ...
             patch("av3.cli.main._wait_for_port", _fake_wait_for_port):
            runner = CliRunner()
    ...
    def test_spawns_serve_with_default_host_port(
        ...
        args = spawned[0]
        # python -m av3.cli.main serve --host 127.0.0.1 --port 9001
        assert args[0] == sys.executable
        assert args[1:4] == ["-m", "av3.cli.main", "serve"]
        assert "--host" in args
        assert "--port" in args
        assert "9001" in args
