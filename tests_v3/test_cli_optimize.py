...
from av3.pipeline.optimize_worker import OptimizeRunSummary
...
def _invoke(tmp_path: Path, monkeypatch, args: list[str], *,
    ...
    monkeypatch.setattr("av3.pipeline.OptimizeWorker", _RecordingWorker, raising=True)
    monkeypatch.setattr(
        "av3.pipeline.optimize_worker.OptimizeWorker", _RecordingWorker, raising=True
    )
