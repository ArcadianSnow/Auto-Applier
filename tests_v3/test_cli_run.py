...
from av3.pipeline.scheduler import CycleSummary, SchedulerRunSummary
...
def _invoke(
    ...
    *,
    ...
    monkeypatch.setattr("av3.pipeline.Scheduler", _RecordingScheduler, raising=True)
    monkeypatch.setattr(
        "av3.pipeline.scheduler.Scheduler", _RecordingScheduler, raising=True
    )
    monkeypatch.setattr(
        "av3.sources.browser.session.BrowserSession",
        _StubBrowserSession,
        raising=True,
    )
...
def test_session_stops_on_scheduler_error(tmp_path, monkeypatch):
    ...
    _StubBrowserSession.started = 0
    _StubBrowserSession.stopped = 0
    monkeypatch.setattr("av3.pipeline.Scheduler", _RaisingScheduler, raising=True)
    monkeypatch.setattr("av3.pipeline.scheduler.Scheduler", _RaisingScheduler, raising=True)
    monkeypatch.setattr(
        "av3.sources.browser.session.BrowserSession",
        _StubBrowserSession,
        raising=True,
