...
:class:`av3.web.service.SchedulerService` between paused and running so
...
from av3.web.control import SOURCE_HOTKEY
...
class HotkeyWatcher:
    ...
    asyncio event loop. The :class:`av3.web.service.SchedulerService`
    methods it calls (``toggle`` / ``pause`` / ``resume``) all flip the
    thread-safe :class:`av3.web.control.ControlState`, so this is safe by
    design.
