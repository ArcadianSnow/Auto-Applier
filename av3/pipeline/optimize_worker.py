...
The state machine (``av3.domain.state``) already lists ``DECIDED → {QUEUED_APPLY,
...
Both are derived from ``job.id`` via :func:`av3.resume.generate.generated_resume_path`
and :func:`av3.resume.generate.generated_cover_letter_path`, so the apply worker
...
from av3.resume.guard import GuardResult, Verdict, guard_l1
from av3.resume.render import PdfRenderer, build_resume_html, render_resume_pdf
...
class OptimizeWorker:
    ...
    production the default :func:`av3.resume.render.render_resume_pdf` wires the
    ...
    versioned prompts that live in :mod:`av3.llm.prompts`.
    """
