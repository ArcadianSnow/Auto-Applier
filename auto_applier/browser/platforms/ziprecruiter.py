...
class ZipRecruiterPlatform(JobPlatform):
    ...
    async def _handle_resume_upload(self, page: Page, resume_path: str) -> None:
        ...
        # Convert .docx / .txt to PDF up front; ZipRecruiter accepts
        # both but consistently parses PDFs better.
        try:
            resume_path = str(await ensure_pdf(resume_path))
        except Exception as exc:
            logger.warning(
                "ZipRecruiter: ensure_pdf failed (%s); using original.",
                exc,
            )

        target = await FormFiller.pick_resume_input(page, "ZipRecruiter")
        if target is None:
            return
