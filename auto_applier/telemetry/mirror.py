...
2. **Schema is intentionally tiny.** ``category`` distinguishes the two §9
   ...
   :mod:`auto_applier.telemetry.scrub`.
...
# here + a scrubber in :mod:`auto_applier.telemetry.scrub`.
_SCRUBBERS = {
...
class MirrorPolicy:
    ...
    def from_settings(cls, telemetry_cfg, app_version: str) -> "MirrorPolicy":
        """Build from a :class:`auto_applier.config.TelemetryConfig` + app version string.
